...
public class StageAddPresenter extends BasePresenter implements StageAddContract.Presenter {
    private final StageAddContract.View mView;
    ServiceRepository repository;

    public StageAddPresenter(StageAddContract.View view,
                             ServiceRepository repository) {
        mView = view;
    ...
    @Override
    public void addStage(Integer accId, Integer stageType) {
        Map<String, Object> map = new HashMap();
        map.put("accId", accId);
        map.put("stageType", stageType);
        mView.showProgressDialog("开通驿站...");
        Disposable disposable = repository.addStage(map)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new DisposableObserver<BaseResponseReturnValue<AddStageResponseBean>>() {
                    @Override
                    public void onNext(BaseResponseReturnValue<AddStageResponseBean> value) {
                        KLog.e(new Gson().toJson(value));
                        mView.dismissProgressDialog();
                        switch (value.getCode()) {
                            case ResponseCode.SUCCESS_CODE:
                                mView.showToast("开通驿站成功");
                                mView.addSuccess(value.getData());
                                break;
    ...
    @Override
    public void createECard(Integer accId, String stageCode, String familyNum) {
        String msg = PresenterUtils.INSTANCE.checkParamasNotNull(
                true,
                new String[]{"用户ID", "驿站code", "数量"},
                accId, stageCode, familyNum);
        if (msg != null) {
            mView.showToast(msg);
            return;
        }
