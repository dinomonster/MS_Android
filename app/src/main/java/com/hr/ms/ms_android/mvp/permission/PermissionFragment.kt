package com.hr.ms.ms_android.mvp.permission
...
import com.hr.ms.ms_android.mvp.permission.resourcemanager.ResourceManagerActivity
import com.hr.ms.ms_android.mvp.permission.usermanager.UserManagerActivity
 ...
 */
class PermissionFragment : BaseAppCompatFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(AccountHelper.getMenuAuth().isUserManagerEnable)data.add(MainIconBean(0,R.mipmap.ic_launcher,"用户管理"))
        if(AccountHelper.getMenuAuth().isRoleManagerEnable)data.add(MainIconBean(1,R.mipmap.ic_launcher,"角色管理"))
        if(AccountHelper.getMenuAuth().isMenuManagerEnable)data.add(MainIconBean(2,R.mipmap.ic_launcher,"菜单管理"))
        adapter.showSinglePage(data)
        adapter.onItemClickListener = BaseQuickAdapter.OnItemClickListener { _, _, position ->
            when(position){
                0->nextActivity(UserManagerActivity::class.java)
                2->nextActivity(ResourceManagerActivity::class.java)
            }
        }
