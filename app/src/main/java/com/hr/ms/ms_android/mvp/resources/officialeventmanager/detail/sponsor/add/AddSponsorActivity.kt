...
import kotlinx.android.synthetic.main.addsponsor_activity.*
import kotlinx.android.synthetic.main.toolbar_layout.*


class AddSponsorActivity : BaseActivity(), View.OnClickListener, AddSponsorContract.View {
    private var presenter: AddSponsorPresenter? = null
    private var bean: Sponsor? = null
    ...
    override fun setViewId(): Int {
        return R.layout.addsponsor_activity
    }
    ...
    override fun initData() {
        ...
        presenter = AddSponsorPresenter(this, ServiceRepository.getInstance(ServiceRemoteDataSource.getInstance(), ServiceLocalDataSource.getInstance()))
    ...
    fun setData() {
        ...
        if (userBean != null) {
            ...
            guestId = bean?.sponsorId
            save_tv.text = "保存"
            ImageLoadUtils.loadCropCircleImage(this, bean?.sponsorUserImg, iv_image)
            ImageLoadUtils.loadCropCircleImage(this, bean?.sponsorLogo, logo_iv)
            imageurl = bean?.sponsorLogo
            name_tv.text = bean?.sponsorUserName
            name_et.setText(bean?.sponsorName)
            url_et.setText(bean?.sponsorUrl)
            freenum_et.setText(bean?.freeNum?.toString())
            edit(false)

            name_et.addTextChangedListener(TextChangeListener().textChange { p0, p1, p2, p3 -> edit(p0.toString() != bean?.sponsorUserName) })
            url_et.addTextChangedListener(TextChangeListener().textChange { p0, p1, p2, p3 -> edit(p0.toString() != bean?.sponsorUrl) })
    ...
    override fun onClick(p0: View?) {
        when (p0) {
            save_tv -> {
                ...
                if (TextUtils.isEmpty(bean?.sponsorInfoId)) {
                    presenter?.addEventSponor(intent.getStringExtra(CommonConstants.ID), userBean?.userId?.toString(), name_et.text.toString(), imageurl, url_et.text.toString(), freenum_et.text.toString())
                } else {
                    presenter?.editEventSponorInfo(bean?.sponsorInfoId, name_et.text.toString(), imageurl, url_et.text.toString(), freenum_et.text.toString())
                }
            }
            chooseimg_ll -> {
